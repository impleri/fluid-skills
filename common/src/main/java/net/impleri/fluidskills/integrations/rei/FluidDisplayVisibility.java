...
public class FluidDisplayVisibility implements DisplayVisibilityPredicate {
    ...
    private boolean isntProducible(Fluid fluid) {
        return !producibility.computeIfAbsent(fluid, ClientRestrictions.INSTANCE::isProducible);
    }

    private boolean isntConsumable(Fluid fluid) {
        return !consumability.computeIfAbsent(fluid, ClientRestrictions.INSTANCE::isConsumable);
    }
